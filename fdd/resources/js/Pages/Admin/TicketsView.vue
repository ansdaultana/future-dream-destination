...
const totalPages = computed(() => Math.ceil(tickets.value.length / info.value.perPage));

const done = (id) => {
    form.post(`/dashboard/ticket-ticket-responded/${id}`);
}
                ...
                <ul role="list"
                    ...
                    <li v-for="ticket in tickets" :key="ticket.ID"
                        ...
                        <div class="flex ml-5 w-36 hidden md:block ">
                            <span v-text="ticket.title"></span>
                        </div>
                        <div class="flex ml-5 w-36 hidden md:block ">
                            <span v-text="ticket.title"></span>
                        </div>
                        <div class="w-16 ml-2 hidden md:block" v-text="ticket.no_of_adults">
                        </div>
                        <div class="w-16  ml-2 hidden md:block" v-text="ticket.no_of_children">
                        </div>

                        <div class="w-40 ml-2 md:pl-8 hidden lg:block">
                            {{ formatUpdatedAt(ticket.created_at) }}

                        </div>
                        <div class="w-20 h-10 text-black hover:text-black justify-between  flex mr-8 ml-2 gap-3">
                            <div @click.prevent="getWhatsAppLink(ticket.phone)"
